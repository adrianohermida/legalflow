...
export const handler: Handler = async (event, context) => {
  ...
  try {
    ...
    console.log(
      `Received Stripe event: ${stripeEvent.type} (${stripeEvent.id})`,
    );

    // Check if event is already processed (idempotency)
    const { data: shouldProcess } = await supabase.rpc("stripe_record_event", {
      p_event: stripeEvent,
    });
    ...
    try {
      switch (stripeEvent.type) {
        ...
        case "customer.updated":
          await supabase.rpc("stripe_upsert_customer", {
            p: stripeEvent.data.object,
          });
          processed = true;
          break;
        ...
        case "product.updated":
          await supabase.rpc("stripe_upsert_product", {
            p: stripeEvent.data.object,
          });
          processed = true;
          break;
        ...
        case "price.created":
        case "price.updated":
          await supabase.rpc("stripe_upsert_price", {
            p: stripeEvent.data.object,
          });
          processed = true;
          break;
        ...
        case "customer.subscription.deleted":
          await supabase.rpc("stripe_upsert_subscription", {
            p: stripeEvent.data.object,
          });
          processed = true;
          break;
        ...
        case "invoice.finalized":
          await supabase.rpc("stripe_upsert_invoice", {
            p: stripeEvent.data.object,
          });
          processed = true;
          break;
        ...
        case "payment_intent.canceled":
          await supabase.rpc("stripe_upsert_payment_intent", {
            p: stripeEvent.data.object,
          });
          processed = true;
          break;
        ...
        case "checkout.session.expired":
          await supabase.rpc("stripe_upsert_checkout_session", {
            p: stripeEvent.data.object,
          });
          processed = true;
          break;
        ...
        case "customer.subscription.trial_will_end":
          console.log(
            `Trial ending for subscription: ${stripeEvent.data.object.id}`,
          );
        ...
        case "invoice.upcoming":
          console.log(
            `Upcoming invoice for customer: ${stripeEvent.data.object.customer}`,
          );
          // Could trigger notifications here
          processed = true;
          break;
      ...
      if (
        stripeEvent.type.startsWith("payment_intent.") ||
        stripeEvent.type.startsWith("invoice.payment_")
      ) {
...
async function handleSubscriptionLifecycle(stripeEvent: any) {
  ...
  try {
    ...
    console.log(
      `Subscription lifecycle handled: ${subscription.id} -> ${subscription.status}`,
    );
  } catch (error) {
...
async function handlePaymentLifecycle(stripeEvent: any) {
  ...
  try {
    ...
    console.log(
      `Payment lifecycle handled: ${stripeEvent.type} for ${eventData.id}`,
    );
  } catch (error) {
