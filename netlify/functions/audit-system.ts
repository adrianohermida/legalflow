      ...
      checks: [
        {
          ...
          status: apiEndpoints && apiEndpoints.length > 0 ? 'ok' : 'error',
          details: apiEndpoints && apiEndpoints.length > 0 ? undefined : 'Nenhum endpoint encontrado'
        },
        {
          id: 'api_tokens_valid',
          name: 'Tokens válidos',
      ...
      checks: [
        {
          ...
          status: stageTypes && stageTypes.length > 0 ? 'ok' : 'error',
          details: stageTypes && stageTypes.length > 0 ? undefined : 'Stage types sem nome'
        },
        {
      ...
      checks: [
        {
          ...
          status: publications && publications.length > 0 ? 'ok' : 'error',
          details: publications && publications.length > 0 ? undefined : 'Publicações sem vínculo com processos'
        },
        {
          id: 'inbox_filters',
    ...
    auditResult['stripe'] = {
      ...
      checks: [
        ...
        },
        {
          ...
          name: 'Sincronização ativa',
          status: stripeCustomers && stripeCustomers.length > 0 ? 'ok' : 'error',
          details: stripeCustomers && stripeCustomers.length > 0 ? undefined : 'Nenhum customer sincronizado'
        }
      ]
    };
    ...
    switch (patchCode) {
      case 'api_library_seed':
        ...
        const endpointsToSeed = [
          { name: 'tribunals', endpoint: '/api/tribunals', method: 'GET', description: 'Lista tribunais' },
          { name: 'processes', endpoint: '/api/processes', method: 'GET', description: 'Lista processos' },
          { name: 'clients', endpoint: '/api/clients', method: 'GET', description: 'Lista clientes' }
        ];
        ...
        for (const endpoint of endpointsToSeed) {
          ...
          if (error) {
            result.errors.push(`Erro ao criar endpoint ${endpoint.name}: ${error.message}`);
          } else {
        ...
        result.message = result.success ? 'API endpoints criados com sucesso' : 'Alguns endpoints falharam';
        break;
