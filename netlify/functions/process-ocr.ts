  ...
  try {
    ...
    if (!documentId || !filePath) {
      return {
        statusCode: 400,
        body: JSON.stringify({
          error: "documentId e filePath são obrigatórios",
        }),
      };
    }
    ...
    console.log(
      `🔍 Iniciando OCR para documento ${documentId}, arquivo: ${filePath}`,
    );
...
async function processOCRContent(
  fileData: Blob,
  filePath: string,
): Promise<string> {
  ...
  try {
    if (fileExtension === "pdf") {
      ...
      throw new Error(
        `Tipo de arquivo não suportado para OCR: ${fileExtension}`,
      );
    }
  } catch (error) {
