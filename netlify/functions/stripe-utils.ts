...
async function searchCustomers(event: any, headers: any) {
  ...
  try {
    // Search by email
    const emailSearch = await stripeRequest(
      `/customers?email=${encodeURIComponent(query)}&limit=10`,
    );

    // Search by general query if no email results
    let generalSearch = { data: [] };
    if (emailSearch.data.length === 0) {
      generalSearch = await stripeRequest(
        `/customers/search?query=${encodeURIComponent(`name:'${query}' OR email:'${query}'`)}&limit=10`,
      );
    }
...
async function syncCustomers(event: any, headers: any) {
  try {
    ...
    while (hasMore && totalSynced < 100) {
      // Limit to 100 customers per sync
...
async function syncAllData(event: any, headers: any) {
  try {
    ...
    const customers = await stripeRequest(
      `/customers?limit=50&created[gte]=${thirtyDaysAgo}`,
    );
    ...
    const subscriptions = await stripeRequest(
      `/subscriptions?limit=50&created[gte]=${thirtyDaysAgo}`,
    );
      ...
      body: JSON.stringify({
        ...
        error:
          error instanceof Error
            ? error.message
            : "Falha na sincronização completa",
      }),
    };
  }
}
...
async function createCheckoutSession(event: any, headers: any) {
  ...
  try {
    ...
    const sessionParams = new URLSearchParams({
      ...
      success_url:
        success_url ||
        `${event.headers.origin}/success?session_id={CHECKOUT_SESSION_ID}`,
    ...
    return {
      ...
      body: JSON.stringify({
        ...
        error:
          error instanceof Error
            ? error.message
            : "Falha ao criar checkout session",
      }),
    };
  }
}
