...
import { THEME_CONFIG, APPROVED_BRAND_COLORS, BLOCKED_COLORS, type ThemeMode } from '../lib/theme-config';
...
export function useColorValidation() {
  const isColorApproved = (color: string): boolean => {
    ...
    const allApprovedColors = Object.values(APPROVED_BRAND_COLORS).flatMap(palette => 
  ...
  const getApprovedColor = (requestedColor: string): string => {
    ...
    console.warn(`🔄 Cor não aprovada "${requestedColor}" substituída por cor principal`);
