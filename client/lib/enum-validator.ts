...
export class EnumValidator {
  ...
  static validateOrDefault<T extends keyof typeof RUNTIME_ENUMS>(
    ...
    if (!result.isValid) {
      console.warn(`Invalid ${enumType} value "${value}", using default "${defaultValue}"`);
      return defaultValue;
    }
  ...
  private static findClosestMatch(
    ...
    return minDistance <= Math.max(1, Math.floor(input.length / 2)) ? closest : null;
  }
...
export const sanitizeFormData = (formData: Record<string, any>, entityType: string) => {
  const sanitized = { ...formData };
  
  switch (entityType) {
    case 'ticket':
      if (sanitized.status) {
        sanitized.status = EnumValidator.validateOrDefault('ticket_status', sanitized.status, 'aberto');
      }
      if (sanitized.priority) {
        sanitized.priority = EnumValidator.validateOrDefault('priority', sanitized.priority, 'media');
      }
      if (sanitized.channel) {
        sanitized.channel = EnumValidator.validateOrDefault('channel_type', sanitized.channel, 'sistema');
      }
      break;
      
    case 'activity':
      if (sanitized.status) {
        sanitized.status = EnumValidator.validateOrDefault('activity_status', sanitized.status, 'todo');
      }
      if (sanitized.priority) {
        sanitized.priority = EnumValidator.validateOrDefault('priority', sanitized.priority, 'media');
      }
    ...
    case 'evento':
      if (sanitized.event_type) {
        sanitized.event_type = EnumValidator.validateOrDefault('event_type', sanitized.event_type, 'outros');
      }
      break;
      
    case 'payment':
      if (sanitized.status) {
        sanitized.status = EnumValidator.validateOrDefault('payment_status', sanitized.status, 'pendente');
      }
      break;
  }
...
export const useEnumValidation = () => {
  const validateAndShowError = (enumType: keyof typeof RUNTIME_ENUMS, value: any) => {
...
export const runEnumConsistencyCheck = () => {
  if (import.meta.env.MODE === 'development') {
    ...
    const checkResults = Object.entries(RUNTIME_ENUMS).map(([enumType, values]) => ({
      enumType,
      values: values.slice(),
      count: values.length
    }));
