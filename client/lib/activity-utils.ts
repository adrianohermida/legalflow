...
export function sortActivitiesByPriority(activities: any[]): any[] {
  return activities.sort((a, b) => {
    ...
    const aPriority = ACTIVITY_PRIORITIES.find(p => p.id === a.priority)?.weight || 0;
    const bPriority = ACTIVITY_PRIORITIES.find(p => p.id === b.priority)?.weight || 0;
...
export function calculateActivityStats(activities: any[]): ActivityStats {
  ...
  const overdue = activities.filter(a => isTaskOverdue(a.due_at, a.status)).length;
  ...
  const averageCompletionTime = completedWithDates.length > 0
    ? completedWithDates.reduce((sum, time) => sum + time, 0) / completedWithDates.length
    : 0;
 ...
 */
export function getAvailableTransitions(currentStatus: string): ActivityStatus[] {
  ...
  const availableIds = allTransitions[currentStatus as keyof typeof allTransitions] || [];
...
export function generateActivitySummary(activities: any[]): string {
  ...
  const urgentTasks = activities.filter(a => a.priority === "urgente" && a.status !== "done").length;
