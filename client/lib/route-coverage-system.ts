...
export class RouteCoverageSystem {
  ...
  private initializeRoutes() {
    ...
    const escritorioRoutes: Omit<
      RouteTest,
      "status" | "renderTime" | "lastTested" | "deeplink"
    >[] = [
      {
        path: "/",
  ...
  async testRoute(routeIndex: number): Promise<RouteTest> {
    ...
    try {
      ...
      if (response.ok) {
        ...
        try {
          const perfMetrics =
            await routePerformanceMonitor.measureRoutePerformance(route.path);
          const evaluation =
            routePerformanceMonitor.evaluatePerformance(perfMetrics);
          ...
          console.warn(
            "Performance measurement failed for",
            route.path,
            perfError,
          );
        }
  ...
  async testAllRoutes(
    onProgress?: (route: RouteTest, index: number, total: number) => void,
  ): Promise<RouteTest[]> {
  ...
  getStats(): RouteCoverageStats {
    ...
    const errors = this.routes.filter((route) =>
    ...
    const avgRenderTime =
      testedRenderTimes.length > 0
        ? testedRenderTimes.reduce((sum, time) => sum + time, 0) /
          testedRenderTimes.length
        : 0;
    ...
    const performanceIssues = this.routes.filter(
      (route) =>
        route.performanceStatus &&
        ["poor", "acceptable"].includes(route.performanceStatus),
    ).length;

    const routesAbove500ms = this.routes.filter(
      (route) => route.renderTime > 500,
    ).length;
    ...
    return {
      ...
      coverage_percentage:
        total > 0 ? Math.round((tested.length / total) * 100) : 0,
