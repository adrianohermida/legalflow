...
export class BuilderAPIDiagnostics {
  ...
  private static async checkSingleEndpoint(
    endpoint: string,
  ...
  static checkCredentials(
    ...
    return {
      public_key: {
        ...
        format_valid:
          publicKey && publicKey.length >= 20 && !publicKey.includes("YOUR_"),
        preview: publicKey
  ...
  static async runCompleteDiagnostics(
    ...
    // Check credentials
    if (
      !credentials.public_key.configured ||
      !credentials.private_key.configured
    ) {
      ...
      if (!credentials.private_key.format_valid) {
        recommendations.push(
          "⚠️ Private key format appears invalid (should start with 'bpk-')",
        );
        issues += 1;
      }
    }
    ...
    if (accessibleEndpoints.length === 0) {
      ...
      recommendations.push(
        "⚠️ Direct API endpoint access blocked (likely CORS)",
      );
      recommendations.push(
        "🔧 This is normal in browser environments - fallback API will be used",
      );
      issues += 1; // Less severe than before
    } else if (accessibleEndpoints.length < endpoints.length / 2) {
      recommendations.push(
        "⚠️ Some Builder.io API endpoints are not accessible",
      );
      recommendations.push(
        "🔍 Mixed connectivity - system will adapt accordingly",
      );
    ...
    if (corsIssues.length > 0 && accessibleEndpoints.length > 0) {
      recommendations.push(
        "ℹ️ Some endpoints have CORS limitations (normal for browser environment)",
      );
    ...
    if (issues === 0) {
      ...
      recommendations.push(
        "🎉 System ready - will use real API when possible, fallback when needed",
      );
    } else if (issues <= 1) {
      overallStatus = "issues";
      recommendations.push(
        "🔧 Minor connectivity issues - system fully functional with smart fallbacks",
      );
    } else if (issues <= 2) {
      overallStatus = "issues";
      recommendations.push(
        "⚠️ Some issues detected - mock API will provide full functionality",
      );
    } else {
      overallStatus = "critical";
      recommendations.push(
        "🚨 Major configuration issues - check credentials and network",
      );
    }
...
export async function quickBuilderAPIDiagnostic(): Promise<{
  ...
  try {
    ...
    if (status.healthy && healthCheck.credentials_valid) {
      ...
      healthCheck.fallback_available &&
      (healthCheck.credentials_valid || healthCheck.endpoint_reachable)
    ) {
    ...
    console.log(
      `🎯 Improved diagnostic result: ${diagnosticStatus} - ${status.message}`,
    );
    ...
    try {
      ...
      return {
        ...
        recommendations: [
          hasValidCredentials
            ? "✅ System will work with mock API fallback"
