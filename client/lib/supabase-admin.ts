...
export async function createTableDirectly(
  tableName: string,
  tableSQL: string,
...
export async function createTablesWithAdmin(): Promise<{
  success: boolean;
  error?: string;
  details?: any;
}> {
  if (!isAdminConfigured) {
    return {
      success: false,
      error:
        "Service role key não configurada. Configure VITE_SUPABASE_SERVICE_ROLE_KEY.",
  ...
  try {
    ...
    return {
      success: false,
      error:
        "Tabelas não encontradas. Execute o script SQL manualmente no Supabase SQL Editor.",
      details: {
