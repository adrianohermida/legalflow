...
export class AuditBacklogIntegrator {
   ...
   */
  static async analyzeAuditResults(
    auditResults: any,
  ): Promise<BacklogSuggestion[]> {
   ...
   */
  private static mapCheckToFinding(
    moduleId: string,
    check: any,
  ): AuditFinding | null {
    const findingMap: Record<string, Partial<AuditFinding>> = {
      "route-coverage": {
        ...
        recommendation:
          "Implementar rotas faltantes e corrigir problemas de deeplink",
        estimatedEffort: 4,
        storyPoints: 3,
        builderExecutable: true,
        builderPrompt:
          "Corrigir problemas de roteamento e implementar rotas faltantes para melhorar a navegação do usuário",
      },
      "database-integrity": {
        type: "error",
        category: "database",
      ...
      "api-endpoints": {
        ...
        builderExecutable: true,
        builderPrompt:
          "Implementar otimizações de performance nos endpoints da API mais utilizados",
      },
      "ui-components": {
        type: "improvement",
        category: "ui",
        ...
        recommendation:
          "Melhorar acessibilidade e consistência dos componentes",
        estimatedEffort: 8,
        storyPoints: 5,
        builderExecutable: true,
        builderPrompt:
          "Refatorar componentes UI para melhorar acessibilidade e seguir padrões do design system",
      },
      "security-audit": {
        type: "error",
        category: "security",
      ...
      "performance-metrics": {
        ...
        recommendation:
          "Implementar melhorias de performance baseadas em métricas",
        estimatedEffort: 5,
        storyPoints: 3,
        builderExecutable: true,
        builderPrompt:
          "Otimizar componentes com base nas métricas de performance coletadas",
      },
    };
    ...
    return {
      ...
      description:
        check.details ||
        check.description ||
        `Problema identificado no módulo ${moduleId}`,
      module: moduleId,
      severity,
      ...
      recommendation:
        ...
        "Investigar e corrigir o problema identificado",
  ...
  private static determineSeverity(
    ...
    if (
      check.details?.includes("critical") ||
      check.details?.includes("security")
    ) {
      return "critical";
    }
    if (
      check.details?.includes("performance") ||
      check.details?.includes("error")
    ) {
      return "high";
    }
   ...
   */
  private static evaluateFindingForBacklog(
    finding: AuditFinding,
   ...
   */
  static async createBacklogItemsFromSuggestions(
    ...
    for (const suggestion of suggestions) {
      if (suggestion.autoCreate) {
        try {
          const itemId = await this.createBacklogItem(
            suggestion.finding,
            userId,
          );
          if (itemId) {
            ...
            await autofixHistory.recordModification({
              ...
              changes: [
                `Criado item ${itemId} baseado em finding de auditoria`,
              ],
              success: true,
            });
          }
   ...
   */
  private static async createBacklogItem(
    finding: AuditFinding,
    userId?: string,
  ): Promise<string | null> {
    try {
  ...
  static async getIntegrationStats(): Promise<{
    ...
    try {
      ...
      auditItems?.forEach((item) => {
        ...
        stats.bySeverity[item.priority] =
          (stats.bySeverity[item.priority] || 0) + 1;
      });
...
export const useAuditBacklogIntegration = () => {
  const processAuditResults = async (auditResults: any) => {
    const suggestions =
      await AuditBacklogIntegrator.analyzeAuditResults(auditResults);
    return suggestions;
  };

  const createItemsFromSuggestions = async (
    suggestions: BacklogSuggestion[],
  ) => {
    return await AuditBacklogIntegrator.createBacklogItemsFromSuggestions(
      suggestions,
    );
  };
