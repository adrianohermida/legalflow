...
export class SchemaValidator {
  ...
  async validateAllEnums(): Promise<SchemaValidationResult> {
        ...
        if (!result.isValid) {
          if (result.error) {
            ...
            if (result.missingInDB.length > 0) {
              errors.push(`${enumName}: Missing in DB: ${result.missingInDB.join(', ')}`);
            }
            if (result.extraInDB.length > 0) {
              warnings.push(`${enumName}: Extra in DB: ${result.extraInDB.join(', ')}`);
            }
          }
        }
      } catch (error: any) {
    ...
    console.log(`✅ Schema validation complete: ${validEnums}/${results.length} enums valid`);
   ...
   */
  async validateEnum(enumName: string, tsValues: readonly string[]): Promise<EnumValidationResult> {
    try {
      // Check cache first
      const cached = this.validationCache.get(enumName);
      if (cached && this.lastValidation && Date.now() - this.lastValidation.getTime() < 5 * 60 * 1000) {
        return cached;
      }
  ...
  private async getEnumValuesFromDB(enumName: string): Promise<string[]> {
    ...
    const enumMappings: Record<string, Array<{ table: string; column: string }>> = {
      ...
      conversation_status: [{ table: 'conversation_properties', column: 'status' }]
    };
    ...
    try {
      ...
      if (error) {
        console.warn(`Could not fetch values from ${table}.${column}:`, error.message);
        // Return TypeScript values as fallback
        return Array.from(TYPESCRIPT_ENUMS[enumName as keyof typeof TYPESCRIPT_ENUMS]);
      }
      ...
      if (values.length === 0) {
        return await this.testEnumValues(table, column, Array.from(TYPESCRIPT_ENUMS[enumName as keyof typeof TYPESCRIPT_ENUMS]));
      }
   ...
   */
  private async testEnumValues(table: string, column: string, testValues: string[]): Promise<string[]> {
  ...
  async generateTypeDefinitions(): Promise<string> {
    ...
    for (const [enumName, tsValues] of Object.entries(TYPESCRIPT_ENUMS)) {
      const values = Array.from(tsValues).map(v => `'${v}'`).join(' | ');
