...
export async function createConfirmedTestUser() {
  if (!supabaseConfigured) {
    console.log(
      "🔧 Skipping test user creation - Supabase not configured (using demo mode)",
    );
    return false;
  }
  ...
  try {
    ...
    console.log(
      "Note: You may need to manually confirm this email in the Supabase dashboard",
    );

    return true;
  } catch (error: any) {
    console.error(
      "Error creating confirmed test user:",
      error.message || error,
    );
    return false;
  }
