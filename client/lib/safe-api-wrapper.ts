...
export class SafeAPIWrapper {
  static async safeAPICall<T>(
    ...
    try {
      ...
      const result = await Promise.race([safeApiCall(), timeoutPromise]);
      ...
      console.log(
        `🔄 Safe API: ${operationName} failed, using fallback:`,
        errorMessage,
      );
...
export const safeBuilderAPICall =
  SafeAPIWrapper.safeBuilderAPICall.bind(SafeAPIWrapper);
export const safeDatabaseOperation =
  SafeAPIWrapper.safeDatabaseOperation.bind(SafeAPIWrapper);
export const safeNetworkOperation =
  SafeAPIWrapper.safeNetworkOperation.bind(SafeAPIWrapper);
