...
const FallbackPage = ({ name }: { name: string }) => (
  <div style={{
    ...
    <p>A página <strong>{name}</strong> está sendo carregada...</p>
...
function SafeApp() {
  ...
  useEffect(() => {
    const loadComponents = async () => {
      ...
      console.log(`🎉 Safe loading complete. Loaded: ${Object.keys(loaded).length}, Errors: ${errors.length}`);
    };
  ...
  const Dashboard = loadedComponents.Dashboard?.Dashboard || (() => <FallbackPage name="Dashboard" />);
  const DashboardV2 = loadedComponents.DashboardV2?.DashboardV2 || (() => <FallbackPage name="Dashboard V2" />);
  const Processos = loadedComponents.Processos?.Processos || (() => <FallbackPage name="Processos" />);
  const Clientes = loadedComponents.Clientes?.Clientes || (() => <FallbackPage name="Clientes" />);
  
  const AppShell = loadedComponents.AppShell?.AppShell || (({ children }: { children: React.ReactNode }) => (
  ...
  const AuthProvider = loadedComponents.AuthContext?.AuthProvider || (({ children }: { children: React.ReactNode }) => children);
              ...
              <Routes>
                ...
                <Route path="*" element={
                  <div style={{ padding: "40px", textAlign: "center" }}>
                    ...
                    <p>Sistema carregado com {Object.keys(loadedComponents).length} componentes</p>
                    {loadErrors.length > 0 && (
                      <div style={{ 
                        marginTop: "20px", 
                        padding: "15px", 
