...
export function ForgotPassword() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (authMode === "demo") {
      setMessage(
        "No modo demonstração, a recuperação de senha não está disponível. Use as credenciais de teste: admin.test@gmail.com / 123456",
      );
      setIsLoading(false);
      return;
    }
    ...
    if (!supabaseConfigured) {
      setError(
        "Supabase não está configurado. A recuperação de senha não está disponível.",
      );
      setIsLoading(false);
      return;
    }
          ...
          <div className="text-center mt-6">
            <Alert className="bg-brand-50 border-brand-300">
              <AlertDescription className="text-brand-700 text-sm">
                <strong>Modo Demonstração:</strong> Use as credenciais de teste
