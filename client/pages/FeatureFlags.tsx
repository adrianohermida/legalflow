...
const FeatureFlags: React.FC = () => {
  ...
  const killSwitchActive = featureFlags?.find(f => f.key === 'kill_switch')?.enabled || false;
  ...
  const toggleAllFeatures = async (enable: boolean) => {
    const regularFeatures = featureFlags?.filter(f => f.key !== 'kill_switch') || [];
  ...
  const enabledCount = featureFlags?.filter(f => f.enabled && f.key !== 'kill_switch').length || 0;
  const totalCount = featureFlags?.filter(f => f.key !== 'kill_switch').length || 0;
    ...
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      ...
      {killSwitchActive && (
        <Alert className="mb-6 border-red-200 bg-red-50">
          ...
          <AlertDescription className="text-red-800">
            <strong>SISTEMA EM MODO SOMENTE LEITURA</strong> - Kill Switch ativo. 
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-3">
              <Switch
                ...
                onCheckedChange={(enabled) => updateFeatureFlag('kill_switch', enabled)}
            ...
            <Alert className="mt-4 border-red-200 bg-red-50">
              ...
              <AlertDescription className="text-red-800">
                <div className="space-y-3">
                  <strong>ATENÇÃO:</strong> Você está prestes a ativar o Kill Switch. 
                  Isso colocará todo o sistema em modo somente leitura.
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {featureFlags?.filter(flag => flag.key !== 'kill_switch').map((flag) => {
            const config = featureConfig[flag.key as keyof typeof featureConfig];
            if (!config) return null;

            return (
              <Card key={flag.key} className={`transition-all ${flag.enabled ? 'border-green-200' : 'border-gray-200'}`}>
                <CardContent className="p-6">
                  ...
                  {!flag.enabled && (
                    <div className="text-xs text-orange-600 mt-2">
                      ⚠️ Usuários serão redirecionados com mensagem informativa
                    </div>
                  )}
                </CardContent>
              </Card>
      ...
      <Card className="mt-8">
        ...
        <CardContent className="text-sm text-gray-600 space-y-2">
          <p>• <strong>Feature Flags:</strong> Habilitam/desabilitam módulos específicos</p>
          <p>• <strong>Kill Switch:</strong> Bloqueia todas as operações de escrita no sistema</p>
          <p>• <strong>Redirecionamento:</strong> Usuários são informados quando tentam acessar módulos desabilitados</p>
          <p>• <strong>Rollback:</strong> Mudanças podem ser revertidas instantaneamente</p>
          <p>• <strong>Logs:</strong> Todas as alterações são registradas para auditoria</p>
        </CardContent>
      </Card>
    </div>
  );
