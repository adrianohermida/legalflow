...
const getStageIcon = (stageType: string) => {
  switch (stageType) {
    ...
    case 'gate': return <CheckCircle className="w-5 h-5" />;
...
const NextActionCard: React.FC<{
  ...
  const getActionIcon = (type: string) => {
    switch (type) {
      ...
      case 'gate': return <CheckCircle className="w-6 h-6" />;
  ...
  const getActionColor = (type: string) => {
    switch (type) {
      ...
      case 'upload': return 'from-orange-500 to-orange-600';
      case 'meeting': return 'from-purple-500 to-purple-600';
      case 'gate': return 'from-red-500 to-red-600';
    ...
    <Card className="border-blue-200 bg-gradient-to-r from-blue-50 to-indigo-50">
      <CardContent className="p-6">
        <div className="flex items-start gap-4">
          <div className={`w-12 h-12 rounded-xl bg-gradient-to-r ${getActionColor(nextAction.type)} flex items-center justify-center text-white shadow-lg`}>
          ...
          <div className="flex-1">
            ...
            {nextAction.due_at && (
              <div className="flex items-center gap-2 text-sm text-neutral-600 mb-4">
                ...
                <span>Prazo: {new Date(nextAction.due_at).toLocaleDateString("pt-BR", {
                  weekday: 'long',
                  year: 'numeric',
                  month: 'long',
                  day: 'numeric'
      ...
      <DialogContent>
          ...
          {action.type === 'lesson' && (
            <div className="p-4 bg-blue-50 rounded-lg">
              <p className="text-sm text-blue-800">
                📚 Esta é uma etapa de aprendizado. Certifique-se de ter assistido todo o conteúdo antes de marcar como concluída.
              </p>
            </div>
          )}
          ...
          {action.type === 'form' && (
            <div className="p-4 bg-green-50 rounded-lg">
              <p className="text-sm text-green-800">
                📝 Preencha o formulário necessário antes de marcar esta etapa como concluída.
              </p>
            </div>
          )}
          ...
          {action.type === 'upload' && (
            <div className="p-4 bg-orange-50 rounded-lg">
              <p className="text-sm text-orange-800">
                📎 Envie todos os documentos necessários antes de marcar como concluída.
              </p>
            </div>
          )}
        ...
        <div className="flex gap-2">
          ...
          <Button 
            ...
            className="flex-1"
          >
            {executeMutation.isPending ? "Processando..." : "Marcar como Concluída"}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
...
export function PortalJornada() {
  ...
  const { data: journeyInstance, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (instanceId) {
        ...
        query = lf.from("vw_process_journey").select("*")
        ...
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <p className="text-neutral-600 max-w-md mx-auto">
              Você não possui jornadas ativas no momento. Entre em contato com seu advogado para iniciar uma nova jornada.
            </p>
          </CardContent>
        </Card>
      </div>
  ...
  const completedStages = stageInstances?.filter(s => s.status === 'completed').length || 0;
    ...
    <div className="space-y-6 p-6">
      <div>
        ...
        <p className="text-neutral-600 mt-1 flex items-center gap-2">
          ...
          <span>•</span>
          <span>Iniciada em {new Date(journeyInstance.start_date).toLocaleDateString("pt-BR")}</span>
          <span>•</span>
        ...
        <CardContent>
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <span className="text-lg font-bold">{completedStages} de {totalStages}</span>
            </div>
            <Progress value={journeyInstance.progress_pct} className="h-3" />
            <div className="text-center">
              <span className="text-2xl font-bold text-blue-600">{journeyInstance.progress_pct}%</span>
          ...
          {stageInstances && stageInstances.length > 0 ? (
            <Accordion type="single" collapsible className="w-full">
              {stageInstances.map((stage, index) => {
                const isOverdue = stage.status !== 'completed' && new Date(stage.sla_at) < new Date();
                const statusColor = getStageStatusColor(stage.status, isOverdue);
                      ...
                      <div className="flex items-center gap-4 w-full">
                        ...
                        <div className="flex-1 text-left">
                          <div className="flex items-center gap-2">
                            ...
                            {stage.mandatory && <Star className="w-4 h-4 text-amber-500" />}
                    ...
                    <AccordionContent>
                      <div className="ml-14 pt-2">
                        ...
                        <div className="text-xs text-neutral-500">
                          Tipo: {stage.stage_type === 'lesson' ? 'Aula' : 
                                 stage.stage_type === 'form' ? 'Formulário' :
                                 stage.stage_type === 'upload' ? 'Upload de Documentos' :
                                 stage.stage_type === 'meeting' ? 'Reunião' :
                                 stage.stage_type === 'gate' ? 'Aprovação' : 'Tarefa'}
                        </div>
                      </div>
                    </AccordionContent>
                  </AccordionItem>
            ...
            <div className="text-center py-8">
              ...
              <p className="text-neutral-600">Nenhuma etapa definida para esta jornada.</p>
            </div>
          )}
        </CardContent>
      </Card>
