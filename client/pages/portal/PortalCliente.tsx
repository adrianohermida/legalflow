        ...
        <div className="space-y-3">
          ...
          <div className="bg-warning-50 p-4 rounded-lg">
            ...
            <p className="text-sm mb-3">
              Reunião de 1 hora para discutir a estratégia do seu caso com base
...
export function PortalCliente() {
  ...
  const [openStage, setOpenStage] = useState<string | undefined>(
    mockJourneyInstance.current_stage_id,
  );

  // In real app, would fetch journey instance by ID
  const journeyInstance = mockJourneyInstance;

  const currentStage = journeyInstance.stages.find(
    (s) => s.id === journeyInstance.current_stage_id,
  );
  const isOverdue =
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
        ...
        {currentStage && (
          ...
          >
            <AlertCircle
              className={`h-4 w-4 ${isOverdue ? "text-danger-600" : "text-brand-700"}`}
            />
            <AlertTitle>
              {isOverdue ? "Ação em Atraso!" : "Próxima Ação"}
            </AlertTitle>
            <AlertDescription>
              <strong>{currentStage.stage_name}</strong> -{" "}
          ...
          <CardContent>
            ...
            >
              {journeyInstance.stages
                ...
                .map((stage) => {
                  const isActive =
                    stage.id === journeyInstance.current_stage_id;
                  const stageIcon = getStageIcon(stage.stage_type);

                  return (
                    <AccordionItem key={stage.id} value={stage.id}>
                      <AccordionTrigger
                        className={`hover:no-underline ${isActive ? "bg-brand-50 px-4 rounded-lg" : ""}`}
                      >
