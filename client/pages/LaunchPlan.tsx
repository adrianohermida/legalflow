...
const LaunchPlan: React.FC = () => {
  ...
  useEffect(() => {
    const items: ChecklistItem[] = [
      // Database
      {
        ...
        description:
          "Todas as migrações SQL foram aplicadas de forma idempotente",
        status: "pending",
        required: true,
  ...
  useEffect(() => {
    if (!healthMetrics || !performanceMetrics || !qaResults || !featureFlags)
      return;
    ...
    setChecklist((prev) =>
      prev.map((item) => {
        ...
        switch (item.id) {
          case "db-rls":
            const rlsMetric = healthMetrics.find(
              (m) => m.metric_name === "rls_enabled",
            );
            newStatus =
              rlsMetric?.status === "healthy" ? "completed" : "failed";
            break;

          case "perf-processos":
            const processosPerf = performanceMetrics.find(
              (m) => m.route === "/processos",
            );
            newStatus =
              processosPerf?.p95_response_ms < 1000 ? "completed" : "failed";
            break;

          case "perf-tickets":
            const ticketsPerf = performanceMetrics.find(
              (m) => m.route === "/tickets",
            );
            newStatus =
              ticketsPerf?.p95_response_ms < 1000 ? "completed" : "failed";
            break;

          case "perf-jornadas":
            const jornadasPerf = performanceMetrics.find(
              (m) => m.route === "/jornadas",
            );
            newStatus =
              jornadasPerf?.p95_response_ms < 1000 ? "completed" : "failed";
            break;

          case "perf-inbox":
            const inboxPerf = performanceMetrics.find(
              (m) => m.route === "/inbox",
            );
            newStatus =
              inboxPerf?.p95_response_ms < 1000 ? "completed" : "failed";
            break;

          case "e2e-smoke":
            const smokeTests = qaResults.filter((q) => q.test_type === "smoke");
            newStatus = smokeTests.every((t) => t.status === "pass")
              ? "completed"
              : "failed";
            break;
          ...
          case "e2e-scenarios":
            ...
            newStatus =
              e2eTests.length >= 5 && e2eTests.every((t) => t.status === "pass")
                ? "completed"
                : "failed";
            break;
          ...
          case "e2e-rls":
            ...
            newStatus = rlsTests.every((t) => t.status === "pass")
              ? "completed"
              : "failed";
            break;
          ...
          case "flags-activities":
            const activitiesFlag = featureFlags.find(
              (f) => f.key === "activities",
            );
          ...
          case "flags-relatorios":
            const relatoriosFlag = featureFlags.find(
              (f) => f.key === "relatorios",
            );
          ...
          case "flags-financeiro":
            const financeiroFlag = featureFlags.find(
              (f) => f.key === "financeiro",
            );
  ...
  const canGoLive =
    summary.requiredCompletionRate === 100 && summary.failed === 0;
  ...
  const getStatusIcon = (status: string, required: boolean) => {
    ...
    return (
      <AlertTriangle
        className={`h-4 w-4 ${required ? "text-orange-600" : "text-gray-400"}`}
      />
    );
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-5xl">
      <div className="mb-8">
        ...
        <p className="text-gray-600">Checklist Go/No-Go para produção</p>
      </div>
      ...
      <Card className="mb-8">
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold">
                {summary.requiredCompletionRate}%
              </div>
          ...
          <Alert
            className={`${canGoLive ? "border-green-200 bg-green-50" : "border-red-200 bg-red-50"}`}
          >
            {canGoLive ? (
            ...
            <AlertDescription
              className={canGoLive ? "text-green-800" : "text-red-800"}
            >
              <strong>
              ...
              {!canGoLive && (
                <span>
                  {" "}
                  - {summary.requiredTotal - summary.requiredCompleted} item(s)
                  crítico(s) pendente(s)
                </span>
              )}
            </AlertDescription>
          </Alert>
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            {launchDecision.decision !== "pending" && (
              <Alert
                className={`${launchDecision.decision === "go" ? "border-green-200 bg-green-50" : "border-red-200 bg-red-50"}`}
              >
                <AlertDescription>
                  <strong>Decisão:</strong>{" "}
                  {launchDecision.decision.toUpperCase()} em{" "}
                  {new Date(launchDecision.timestamp).toLocaleString("pt-BR")}
