...
const SLATicketsReport: React.FC = () => {
  ...
  const { data: tickets, isLoading } = useQuery({
    ...
    queryFn: async (): Promise<{ data: TicketMetric[]; total: number }> => {
      ...
      if (filters.search) {
        query = query.or(
          `title.ilike.%${filters.search}%,cliente_cpfcnpj.ilike.%${filters.search}%`,
        );
      }
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-neutral-600">
              Análise detalhada de FRT/TTR por ticket
            </p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setFilters({ ...filters, group_key: value })
                }
              >
                <SelectTrigger>
                  <SelectValue />
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between">
                  <div className="text-sm text-neutral-600">
                    ...
                    {Math.min(currentPage * itemsPerPage, tickets?.total || 0)}{" "}
                    de {tickets?.total || 0} tickets
                  </div>
                  <div className="flex gap-2">
                    <Button
                      variant="outline"
