...
const Relatorios: React.FC = () => {
  ...
  const { data: slaTicketsData } = useQuery({
    ...
    queryFn: async (): Promise<SLATicketsData> => {
      ...
      const avgFrt =
        data.reduce((acc, t) => acc + (t.frt_minutes || 0), 0) / total || 0;
      const avgTtr =
        data.reduce((acc, t) => acc + (t.ttr_minutes || 0), 0) / total || 0;
      ...
      return {
        ...
        frt_violation_rate:
          total > 0 ? ((total - frtCompliant) / total) * 100 : 0,
        ttr_violation_rate:
          total > 0 ? ((total - ttrCompliant) / total) * 100 : 0,
  ...
  const { data: csatData } = useQuery({
    ...
    queryFn: async (): Promise<CSATData> => {
      ...
      const avgRating =
        data.reduce((acc, d) => acc + (d.csat_avg || 0), 0) / data.length || 0;
      const totalResponses = data.reduce(
        (acc, d) => acc + (d.responses || 0),
        0,
      );
  ...
  const metrics: MetricCard[] = [
    {
      ...
      color:
        (slaTicketsData?.frt_violation_rate || 0) > 15
    ...
    },
    {
      ...
      color:
        (financialData?.total_overdue || 0) > 0
          ? "border-red-200 bg-red-50"
          : "border-green-200 bg-green-50",
    },
  ];
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-red-700 mt-1">
                {(slaTicketsData?.frt_violation_rate || 0) > 20 &&
                  "Alto índice de violação de SLA em tickets. "}
                {(slaEtapasData?.overdue || 0) > 5 &&
                  "Múltiplas etapas de jornada em atraso. "}
                {(financialData?.total_overdue || 0) > 10000 &&
                  "Valor significativo em atraso no financeiro. "}
          ...
          <CardContent className="space-y-4">
            <div className="space-y-3">
              ...
              <Progress
                value={
                  slaTicketsData
                    ? (slaTicketsData.frt_compliant / slaTicketsData.total) *
                        100 || 0
                    : 0
                }
                className="h-2"
            ...
            <div className="space-y-3">
              ...
              <Progress
                value={
                  slaTicketsData
                    ? (slaTicketsData.ttr_compliant / slaTicketsData.total) *
                        100 || 0
                    : 0
                }
                className="h-2"
              ...
              <div className="flex items-center gap-3 p-2 bg-green-50 rounded">
                ...
                <div className="text-sm">
                  ...
                  <p className="text-neutral-600">
                    Últimos 7 dias com 85% de compliance
                  </p>
                </div>
              </div>
              ...
              {(financialData?.overdue_installments || 0) > 0 && (
                <div className="flex items-center gap-3 p-2 bg-orange-50 rounded">
                  ...
                  <div className="text-sm">
                    ...
                    <p className="text-neutral-600">
                      {financialData?.overdue_installments} parcelas precisam de
                      atenção
                    </p>
                  </div>
                </div>
