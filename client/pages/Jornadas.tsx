...
const mockTemplates: JourneyTemplate[] = [
  {
    ...
    description: 'Processo completo para novos clientes com questões trabalhistas',
    nicho: 'Trabalhista',
    ...
    stages: [
      { id: '1', template_id: '1', name: 'Coleta de Documentos', description: 'Upload de carteira de trabalho e documentos', stage_type: 'upload', sequence_order: 1, is_required: true, sla_hours: 48, estimated_days: 2, rules: [] },
      { id: '2', template_id: '1', name: 'Análise do Caso', description: 'Formulário de análise inicial', stage_type: 'form', sequence_order: 2, is_required: true, sla_hours: 24, estimated_days: 1, rules: [] },
      { id: '3', template_id: '1', name: 'Reunião Estratégica', description: 'Primeira reunião para definir estratégia', stage_type: 'meeting', sequence_order: 3, is_required: true, sla_hours: 168, estimated_days: 7, rules: [] }
    ]
  },
  {
    id: '2',
    ...
    created_by_oab: '123456/SP',
    stages: [
      { id: '4', template_id: '2', name: 'Orientação Inicial', description: 'Vídeo explicativo sobre o processo', stage_type: 'lesson', sequence_order: 1, is_required: true, sla_hours: 24, estimated_days: 1, rules: [] },
      { id: '5', template_id: '2', name: 'Documentação Pessoal', description: 'Upload de documentos pessoais e do cônjuge', stage_type: 'upload', sequence_order: 2, is_required: true, sla_hours: 72, estimated_days: 3, rules: [] }
    ]
  },
  {
    id: '3',
    ...
    created_by_oab: '123456/SP',
    stages: [
      { id: '6', template_id: '3', name: 'Auditoria Financeira', description: 'Levantamento completo da situação financeira', stage_type: 'form', sequence_order: 1, is_required: true, sla_hours: 240, estimated_days: 10, rules: [] }
    ]
  }
];
...
export function Jornadas() {
  ...
  const filteredTemplates = mockTemplates.filter(template => {
    ...
    const matchesNicho = nichoFilter === 'todos' || template.nicho === nichoFilter;
    return matchesSearch && matchesNicho;
  });

  const filteredInstances = mockInstances.filter(instance => {
    const matchesSearch = instance.cliente_nome.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         instance.template_name.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesStatus = statusFilter === 'todos' || instance.status === statusFilter;
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'paused': return <PauseCircle className="h-4 w-4 text-warning-600" />;
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 mt-1">Gerencie templates e instâncias de jornadas do cliente</p>
        </div>
            ...
            {filteredInstances.map((instance) => (
              <Card key={instance.id} className="hover:shadow-md transition-shadow">
                <CardHeader>
                  <div className="flex justify-between items-start">
                    <div>
                      <CardTitle className="text-lg">{instance.template_name}</CardTitle>
                      <CardDescription className="mt-1">
                  ...
                  <div className="space-y-4">
                    ...
                    {instance.processo_numero_cnj && (
                      <div className="text-sm">
                        ...
                        <p className="font-mono text-xs">{instance.processo_numero_cnj}</p>
                      </div>
                    )}
