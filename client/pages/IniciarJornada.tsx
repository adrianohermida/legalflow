...
export function IniciarJornada() {
  ...
  useEffect(() => {
    if (formData.cliente_cpfcnpj) {
      const clienteProcessos = mockProcessos.filter(
        (p) => p.cliente_cpfcnpj === formData.cliente_cpfcnpj,
      );
  ...
  const selectedCliente = mockClientes.find(
    (c) => c.cpfcnpj === formData.cliente_cpfcnpj,
  );
  const selectedProcesso = mockProcessos.find(
    (p) => p.numero_cnj === formData.processo_numero_cnj,
  );
  const selectedAdvogado = mockAdvogados.find(
    (a) => a.oab === formData.responsavel_oab,
  );
