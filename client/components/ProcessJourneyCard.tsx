...
const getStageIcon = (stageType: string) => {
  switch (stageType) {
    ...
    case 'gate': return <CheckCircle className="w-4 h-4" />;
        ...
        <div className="py-4">
          ...
          <div className="mt-4 p-3 bg-neutral-50 rounded-lg">
            ...
            <div className="text-sm text-neutral-600 mt-1">
              Esta ação irá atualizar o progresso da jornada e calcular a próxima ação.
            </div>
          </div>
        </div>
              ...
              <SelectContent>
                {templates?.map((template) => (
                  <SelectItem key={template.id} value={template.id}>
                    {template.name} ({template.niche} - {template.eta_days} dias)
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
...
export const ProcessJourneyCard: React.FC<{ numeroCnj: string }> = ({ numeroCnj }) => {
  ...
  const handleRefresh = () => {
    refetch();
    queryClient.invalidateQueries({ queryKey: ["journey-stages", journey?.id] });
  };
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-center py-8">
            ...
            <h3 className="font-medium text-neutral-900 mb-2">Nenhuma jornada ativa</h3>
  ...
  const progressColor = journey.progress_pct >= 100 ? "bg-green-500" : journey.progress_pct >= 50 ? "bg-blue-500" : "bg-neutral-400";
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Badge variant={journey.status === "active" ? "default" : "secondary"}>
              {journey.status === "active" ? "Ativa" : journey.status === "completed" ? "Concluída" : "Pausada"}
            </Badge>
          </div>
        </div>
      </CardHeader>
          ...
          <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <h4 className="font-medium text-blue-900">{nextAction.title}</h4>
                <p className="text-sm text-blue-700 mt-1">{nextAction.description}</p>
                {nextAction.due_at && (
                  <div className="flex items-center gap-1 text-xs text-blue-600 mt-2">
                    <Clock className="w-3 h-3" />
                    Prazo: {new Date(nextAction.due_at).toLocaleDateString("pt-BR")}
                  </div>
                )}
                      ...
                      <div className="ml-11 space-y-3">
                        <div className="text-sm text-neutral-600">
                          Tipo: {stage.stage_type === 'lesson' ? 'Aula' : 
                                 ...
                                 stage.stage_type === 'upload' ? 'Upload' :
                                 stage.stage_type === 'meeting' ? 'Reunião' :
                                 stage.stage_type === 'gate' ? 'Aprovação' : 'Tarefa'}
                        </div>
                        {stage.status !== 'completed' && (
                          <CompleteStageDialog stage={stage} onComplete={handleRefresh} />
                        )}
                      </div>
                    </AccordionContent>
                  </AccordionItem>
