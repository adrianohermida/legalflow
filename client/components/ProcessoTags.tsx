...
const TAG_COLORS = [
  { name: 'Cinza', value: 'gray', class: 'bg-gray-100 text-gray-800 hover:bg-gray-200' },
  { name: 'Azul', value: 'blue', class: 'bg-blue-100 text-blue-800 hover:bg-blue-200' },
  { name: 'Verde', value: 'green', class: 'bg-green-100 text-green-800 hover:bg-green-200' },
  { name: 'Amarelo', value: 'yellow', class: 'bg-yellow-100 text-yellow-800 hover:bg-yellow-200' },
  { name: 'Laranja', value: 'orange', class: 'bg-orange-100 text-orange-800 hover:bg-orange-200' },
  { name: 'Vermelho', value: 'red', class: 'bg-red-100 text-red-800 hover:bg-red-200' },
  { name: 'Roxo', value: 'purple', class: 'bg-purple-100 text-purple-800 hover:bg-purple-200' },
  { name: 'Rosa', value: 'pink', class: 'bg-pink-100 text-pink-800 hover:bg-pink-200' },
];
...
export default function ProcessoTags({ 
  ...
  const addTagMutation = useMutation({
    mutationFn: async ({ tag, color }: { tag: string; color: string }) => {
      ...
      const newTagObj = { name: tag, color, created_at: new Date().toISOString() };
      
      // Check if tag already exists
      const tagExists = currentTags.some((t: any) => 
        (typeof t === 'string' ? t : t.name || t.tag) === tag
      );
  ...
  const removeTagMutation = useMutation({
    mutationFn: async (tagToRemove: string) => {
      ...
      const currentTags = processo?.data?.tags || [];
      const updatedTags = currentTags.filter((t: any) => 
  ...
  const getTagColor = (tagData: any) => {
    const color = typeof tagData === 'string' ? 'gray' : (tagData.color || 'gray');
          ...
          <DialogContent className="sm:max-w-md">
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-end gap-2 pt-4">
                ...
                >
                  {addTagMutation.isPending ? 'Adicionando...' : 'Adicionar Tag'}
                </Button>
              </div>
            </div>
          </DialogContent>
