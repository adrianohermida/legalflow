...
export default function ProcessoTimelineUnificada({
  numeroCnj,
}: ProcessoTimelineUnificadaProps) {
  const { toast } = useToast();
  const queryClient = useQueryClient();

  // State management
  const [viewMode, setViewMode] = useState<"recent" | "complete">("recent");
  const [showCompleteHistoryModal, setShowCompleteHistoryModal] =
    useState(false);
  const [showLinkPublicationModal, setShowLinkPublicationModal] =
    useState(false);
  ...
  const [selectedPublication, setSelectedPublication] =
    useState<UnlinkedPublication | null>(null);
  ...
  const { data: unlinkedPublications = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (processoSearchTerm) {
        query = query.or(
          `data->>resumo.ilike.%${processoSearchTerm}%,data->>texto.ilike.%${processoSearchTerm}%`,
        );
      }
  ...
  const TimelineEventCard = ({ event }: { event: TimelineEvent }) => (
    <Card
      key={`${event.tipo}-${event.source_id}`}
      className="p-4 hover:shadow-md transition-shadow"
    >
      <div className="flex items-start gap-3">
        <div className="flex-shrink-0 mt-1">{getEventIcon(event.tipo)}</div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Dialog
            open={showCompleteHistoryModal}
            onOpenChange={setShowCompleteHistoryModal}
          >
            ...
            <DialogContent className="max-w-4xl max-h-[80vh] overflow-hidden flex flex-col">
              <DialogHeader>
                <DialogTitle>
                  Histórico Completo - Timeline do Processo
                </DialogTitle>
              </DialogHeader>
                ...
                <div className="border-t pt-4">
                  <Pagination>
                    <PaginationContent>
                      ...
                      {Array.from(
                        {
                          length: Math.min(5, completeTimelineData.totalPages),
                        },
                        (_, i) => {
                          const page = i + 1;
                          return (
                      ...
                      <PaginationItem>
                        <PaginationNext
                          onClick={() =>
                            setCurrentPage(
                              Math.min(
                                completeTimelineData.totalPages,
                                currentPage + 1,
                              ),
                            )
                          }
                          className={
                            currentPage === completeTimelineData.totalPages
                              ? "pointer-events-none opacity-50"
                              : "cursor-pointer"
                          }
                        />
      ...
      {viewMode === "recent" && (
        <div className="flex gap-2 flex-wrap">
          ...
          <Dialog
            open={showLinkPublicationModal}
            onOpenChange={setShowLinkPublicationModal}
          >
            <DialogTrigger asChild>
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-sm font-medium">
                              {pub.data_publicacao
                                ? formatEventDate(pub.data_publicacao)
                                : "Data não informada"}
                            </span>
                          </div>
