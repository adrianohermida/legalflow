...
export function BuilderDebugger() {
  ...
  useEffect(() => {
    const info: BuilderDebugInfo = {
      ...
      environment: window.location.hostname.includes('builder.codes') ? 'Builder.io' : 'Development',
      apiUrls: [
    ...
    const checkForEmptyComponents = () => {
      ...
      if (emptyComponents > 10) {
        console.warn('🚨 Muitos componentes vazios detectados:', emptyComponents);
      }
    };
  ...
  return (
    <div style={{
      ...
      <div>Builder SDK: {debugInfo.builderContext.hasBuilderSDK ? '✅' : '❌'}</div>
