...
export default function SF5JourneyCardTest() {
  ...
  const createTestJourney = async () => {
    ...
    try {
      ...
      const { data: journeyId, error } = await lf.rpc('create_journey_with_stages', {
      ...
      toast({
        title: 'Erro',
        description: error instanceof Error ? error.message : 'Erro ao criar jornada de teste',
        variant: 'destructive',
      });
    } finally {
  ...
  const testProgressUpdates = async (journeyId: string) => {
    ...
    try {
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : 'Erro durante a execução dos testes',
        variant: 'destructive',
      });
    }
  };
