...
-- ============================================
-- File: SQL_PHASE5_CRM_SCHEMA.sql
-- Description: CRM schema with unified contacts, leads conversion, deals pipeline and Stripe integration
...
insert into legalflow.app_events (event, payload) values (
  'phase5_crm_schema_created',
  jsonb_build_object(
    'contacts_table', 'created',
    'organizations_table', 'created',
    'pipeline_defs_table', 'created',
    'pipeline_stages_table', 'created',
    'deals_updated', 'added_contact_pipeline_fields',
    'functions', 'crm_upsert_contact, crm_convert_lead',
    'view', 'vw_contacts_unified',
    'timestamp', now(),
    'version', '5.0-crm'
  )
);
...
-- ============================================
-- SUMMARY: CRM Schema Patch Complete
-- ============================================

/*
✅ CREATED:
- legalflow.contacts (unified contact management)
- legalflow.organizations (company management)  
- legalflow.org_contacts (contact-organization linking)
- legalflow.pipeline_defs (sales pipeline definitions)
- legalflow.pipeline_stages (pipeline stages)
- legalflow.vw_contacts_unified (view combining public.clientes + contacts)

✅ UPDATED:
- legalflow.deals (added contact_id, pipeline_id, stage_id)

✅ FUNCTIONS:
- legalflow.crm_upsert_contact() (smart contact creation/update)
- legalflow.crm_convert_lead() (lead to contact+deal conversion)

✅ SEEDED:
- Sales pipeline with 5 stages (novo → qualificado → proposta → ganho/perdido)
