...
function showInstructions() {
  ...
  console.log(
    "   - O chat aparece apenas em páginas de processo (/processos/:cnj)",
  );
  console.log("   - Use /dev-auditoria > Processos para testar e gerenciar\n");

  console.log("🔧 TROUBLESHOOTING:");
  console.log(
    "   - Se o chat não aparecer: verifique se o schema foi instalado",
  );
  console.log("   - Se as quick actions falharem: confira as permissões RPC");
  console.log(
    "   - Se houver erros: consulte os logs no componente SF2ProcessosSetup\n",
  );
}
...
function checkSQLFile() {
  const sqlPath = path.join(
    __dirname,
    "..",
    "SF2_CHAT_MULTITHREAD_SCHEMA_COMPLETE.sql",
  );
  ...
  if (fs.existsSync(sqlPath)) {
    ...
    console.log(
      `✅ Arquivo encontrado: SF2_CHAT_MULTITHREAD_SCHEMA_COMPLETE.sql`,
    );
