      ...
      if (statement.trim()) {
        ...
        try {
          ...
          if (error) {
            ...
            if (directError) {
              console.log(
                `  ⚠️  Statement ${i + 1} pode ter falhado: ${error.message}`,
              );
            } else {
...
async function main() {
  ...
  try {
    ...
    const schemaPath = path.join(
      process.cwd(),
      "INBOX_READ_TRACKING_SCHEMA.sql",
    );
    ...
    if (fs.existsSync(schemaPath)) {
      ...
      console.log(
        "⚠️  Arquivo INBOX_READ_TRACKING_SCHEMA.sql não encontrado, pulando...",
      );
    }
    ...
    console.log("\n📋 Próximos passos:");
    console.log(
      "1. Substitua o arquivo InboxLegalV2.tsx pelo InboxLegalV2Enhanced.tsx",
    );
    ...
    console.error(
      "\n❌ Erro durante a aplicação das melhorias:",
      error.message,
    );
    process.exit(1);
  }
