...
async function setupSF6Automation() {
  console.log(
    "🚀 Setting up SF-6: Activities ↔ Tickets Bridge automation...\n",
  );
  ...
  try {
    ...
    const sqlPath = path.join(
      __dirname,
      "..",
      "SF6_AUTO_CREATE_ACTIVITY_RPC.sql",
    );
    ...
    console.log(
      "  • trigger_sf6_auto_create_activity trigger on stage_instances",
    );
    ...
    if (testError) {
      ...
      console.log(
        `  • Processed: ${testResult.processed_count} completed task stages`,
      );
    ...
    console.log(
      "   Activities will be auto-created when task-type stages are completed.",
    );
  } catch (error) {
